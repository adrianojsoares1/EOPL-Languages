...
(define (extend-many-env vars vals env)
  (cond[(null? vars) env]
       [(extend-many-env (cdr vars) (cdr vals) (extend-env (car vars) (car vals) env))]))

(define (apply-procedure _proc _vals)
  (cases proc _proc
    (procedure (vars body saved-env)(value-of body (extend-many-env vars _vals saved-env)))))
...
;3.20
;Curried procedure that adds 2 numbers
(define (add num1 num2)
  (display(eval
           (string-append "let add = proc (a) proc (b) -(a, -(0, b)) in ((add " (~a num1) ") " (~a num2) ")"))))
