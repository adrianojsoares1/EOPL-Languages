...
(define the-grammar
  '((program (expression) a-program)
    ...
    (expression
     ("cond" (arbno "[" expression ":" expression "]")"[else" expression "]")cond-exp)
    ...
    (expression ("gofuckyourself" number) nameless-var-exp)
    
    (expression
     ("kys" expression "moron" expression) nameless-let-exp)
    
    (expression ("dickbutt" expression) nameless-proc-exp)
...
(define (translation-of exp senv)
  (cases expression exp
    ...
    (cond-exp (ifs thens els)
              (cond-exp
               (map (lambda (x) (translation-of x senv)) ifs)
               (map (lambda (x) (translation-of x senv)) thens)
               (translation-of els senv)))
    (var-exp (var)
             (nameless-var-exp
              (apply-senv senv var)))
...
(define (value-of exp nameless-env)
  ...
  (define (navigate-cond ifs thens els)
    (cond[(null? ifs) els]
         [(expval->bool (value-of (car ifs) nameless-env))(value-of (car thens) nameless-env)]
         [else (navigate-cond (cdr ifs) (cdr thens) els)]))
    ...
    (cases expression exp
      ...
      (cond-exp (ifs thens els)(navigate-cond ifs thens els))
...
(for-each (lambda (x) (display(string-append (~a x) "\n"))) examples)
